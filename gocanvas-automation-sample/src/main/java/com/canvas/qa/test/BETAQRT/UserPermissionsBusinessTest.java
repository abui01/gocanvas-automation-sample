...
public class UserPermissionsBusinessTest extends BrowserLaunchTest {

	@Test
	@Parameters({ "testdescription" })

	public void UserPermissionsBusiness(String testDescription, ITestContext testContext)
		...
		try {
			...
			boolean valuesOfWorkflowDispatchLink = dashboardLink.checkForDispatcLink(parameters.get("Dispatch _link"));
			reportLog(valuesOfWorkflowDispatchLink, testContext.getName(),
					"Verify Company Admin can access Dispatch link  ", "1.5",
					"Dispatch Link is verified for Company Admin");
			...
			boolean valuesOfCDWorkflowDispatchLink = dashboardLink
					.checkForDispatcLink(parameters.get("Dispatch _link"));
			reportLog(valuesOfCDWorkflowDispatchLink, testContext.getName(),
					"Verify Designer can access Dispatch link  ", "2.4",
					"Dispatch Link is verified for Company Designer");
			...
			reportLog(!valuesOfCRDispatchLink, testContext.getName(),
					"Verify Company Reporter can access dispatch link  ", "3.7",
					"Company Reporter does not see the Dispatch link");
			...
			reportLog(valuesOfCUSubmissionLink, testContext.getName(),
					"Verify  Company user can access submission link  ", "4.1",
			...
			reportLog(valuesOfCUProfileLink, testContext.getName(), "Verify  Company user can access Profile link  ",
					"4.2", "Profile Link is verified for Company User");
			...
			reportLog(!valuesOfCUdashboardLink, testContext.getName(),
					"Verify Company user can access Dashboard link  ", "4.3",
			...
			reportLog(!valuesOfCUAppLink, testContext.getName(), "Verify Company user can access App link  ", "4.4",
			...
			reportLog(!valuesOfCURefDataAndImageLink, testContext.getName(),
					"Verify Company user can access Reference data & image link  ", "4.5",
			...
			reportLog(!valuesOfCUDispatchLink, testContext.getName(), "Verify Company user can access Dispatch link  ",
					"4.5", "Company User does not see the dispatch link");
			org.testng.Assert.assertFalse(valuesOfCUDispatchLink);

			boolean valuesOfCUReport = dashboardLink.isReportLinkDisplayed();
			reportLog(!valuesOfCUReport, testContext.getName(), "Verify Company user can access Report link  ", "4.6",
			...
			reportLog(!valuesOfCUAccountLink1, testContext.getName(), "Verify Company user can access Account link  ",
					"4.7", "Company User does not see the Account link");
			org.testng.Assert.assertFalse(valuesOfCUAccountLink1);

			boolean editProfile = dashboardLink.updateRole();
			reportLog(!editProfile, testContext.getName(), " Verify the user cannot update the Role from Profile Link ",
					"5.1", "Company User Can't edit role");
