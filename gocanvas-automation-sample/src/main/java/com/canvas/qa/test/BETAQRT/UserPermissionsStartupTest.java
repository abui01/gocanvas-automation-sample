...
public class UserPermissionsStartupTest extends BrowserLaunchTest {

	@Test
	@Parameters({ "testdescription" })

	public void UserPermissionsStartup(String testDescription, ITestContext testContext)
		...
		try {
			performLogin(1, parameters.get("company_admin_username"), parameters.get("company_admin_password"),
					"Login as the Company admin: " + parameters.get("company_admin_username"), testContext,
					testDescription);
			...
			boolean valuesOfWorkflowDispatchLink = dashboardLink
					.checkForWorkflowAndDispatcLink(parameters.get("workflowDispatch_link"));
			reportLog(valuesOfWorkflowDispatchLink, testContext.getName(),
					"Verify Company Admin can access Workflow & Dispatch link  ", "1.5",
					"Workflow & Dispatch Link is verified for Company Admin");
			...
			reportLog(!valuesOfCUAccountLink, testContext.getName(),
					"Verify Company Reporter can access Account link  ", "3.7",
			...
			reportLog(!valuesOfCUAccountLink1, testContext.getName(), "Verify Company user can access Account link  ",
					"4.6", "Company User does not see the Account link");
